%-----------------------------------------------------------------------------%
% Informasi Mengenai Dokumen
%-----------------------------------------------------------------------------%
%
% Judul laporan.
\var{\judul}{JSONField for Django-Supported Databases}
%
% Tulis kembali judul laporan, kali ini akan diubah menjadi huruf kapital
\Var{\Judul}{JSONField for Django-Supported Databases}
%
% Tulis kembali judul laporan namun dengan bahasa Ingris
\var{\judulInggris}{\code{JSONField} for Django-Supported Databases}
%
% Tulis judul dalam bahasa Indonesia
\var{\judulIndonesia}{\code{JSONField} untuk Basis Data yang Didukung Django}

%
% Tipe laporan, dapat berisi Skripsi, Tugas Akhir, Thesis, atau Disertasi
\var{\type}{Bachelor Thesis}
%
% Tulis kembali tipe laporan, kali ini akan diubah menjadi huruf kapital
\Var{\Type}{Bachelor Thesis}
%
% Tulis nama penulis
\var{\penulis}{Sage Muhammad Abdullah}
%
% Tulis kembali nama penulis, kali ini akan diubah menjadi huruf kapital
\Var{\Penulis}{Sage Muhammad Abdullah}
%
% Tulis NPM penulis
\var{\npm}{1706979455}
%
% Tuliskan Fakultas dimana penulis berada
\var{\fakultas}{Computer Science}
\Var{\Fakultas}{Computer Science}
%
% Tuliskan Program Studi yang diambil penulis
\var{\program}{Computer Science Undergraduate}
\Var{\Program}{Computer Science Undergraduate}
%
% Tuliskan Program Studi dalam bahasa Indonesia
\var{\programIndonesia}{Ilmu Komputer}
\Var{\ProgramIndonesia}{Ilmu Komputer}
%
% Tuliskan kota/kabupaten publikasi laporan
\var{\kota}{Depok}
\Var{\Kota}{Depok}
%
% Tuliskan tahun publikasi laporan
\Var{\bulanTahun}{January 2021}
%
% Tuliskan gelar yang akan diperoleh dengan menyerahkan laporan ini
\var{\gelar}{S.Kom.}
%
% Tuliskan tanggal pengesahan laporan, waktu dimana laporan diserahkan ke
% penguji/sekretariat
\var{\tanggalSiapSidang}{26 January 2021}
%
% Tuliskan tanggal keputusan sidang dikeluarkan dan penulis dinyatakan
% lulus/tidak lulus
\var{\tanggalLulus}{18 January 2021}
% Tuliskan tanggal pengesahan laporan final, waktu dimana laporan
% diserahkan ke perpustakaan
\var{\tanggalFinal}{26 January 2021}
%
% Tuliskan pembimbing
\var{\pembimbingSatu}{Dr. Ade Azurat}
\var{\pembimbingDua}{Hafiyyan S. Fadhlillah, S.Kom., M.Kom.}
%
% Tuliskan penguji
\var{\pengujiSatu}{Gladhi Guarddin, S.Kom., M.Kom.}
\var{\pengujiDua}{Dr. Rizal Fathoni Aji, S.Kom., M.Kom.}

% Tulisan untuk menandakan halaman lampiran lanjutan
\var{\continued}{(continued)}

%-----------------------------------------------------------------------------%
% Judul Setiap Bab
%-----------------------------------------------------------------------------%
%
% Berikut ada judul-judul setiap bab.
% Silahkan diubah sesuai dengan kebutuhan.
%
\Var{\kataPengantar}{Preface}
\Var{\babSatu}{Introduction}
\Var{\babDua}{Django Web Framework and JSON}
\Var{\babTiga}{Design and Analysis}
\Var{\babEmpat}{Implementation}
\Var{\babLima}{JSON Data Validation}
\Var{\babEnam}{Results}
\Var{\kesimpulan}{Conclusions}
