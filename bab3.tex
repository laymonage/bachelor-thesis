...
Django recommends avoiding the use of SQL \code{NULL} for \code{JSONField}
...
and the empty string (\code{''}). On \code{JSONField}, it means that there are
the empty JSON object (\code{'\{\}'}), the empty JSON array (\code{'[]'}), the
empty JSON string (\code{'""'}), the JSON \code{null} value (\code{'null'}),
and the SQL \code{NULL}. To avoid redundancy, Django recommends setting
\code{null=False} (to enforce database-level \code{NOT NULL} constraint) and
providing a suitable default for empty values, such as \code{default=dict}.
...
extraction can be done using the \code{JSON\_VALUE} function to extract scalar
values and the \code{JSON\_QUERY} function to extract JSON objects and JSON
...
value is extracted as \code{TEXT} instead of \code{JSONB}.
