...
\code{jsonb} is available as of version 9.4, so \code{JSONField} is always
...
the \code{NCLOB} data type on Oracle Database, so its converter can be reused
for \code{JSONField}.
...
version 12.2 and higher, so \code{JSONField} is always supported. The
