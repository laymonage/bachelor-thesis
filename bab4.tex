...
\listing
{Python}
{The \code{DatabaseWrapper} and \code{DatabaseOperations} classes
of the Oracle Database backend.}
{code:backends-oracle-1}
{codes/4-backends-oracle-1.py}

For the Oracle Database backend, the \verb|DatabaseWrapper| and
\verb|DatabaseOperations| classes are modified to map the correct data types
between Python and the database. The \verb|DatabaseWrapper| class is modified
by updating the \verb|data_types| property to map \verb|JSONField| to
\verb|NCLOB|\footnote{The \code{NCLOB} data type is chosen to allow data larger
than 32767 bytes and to have Unicode support \cite{oracle:overview-json}.}
(shown by line 7 of \autoref{code:backends-oracle-1}) and updating the
\verb|data_type_check_constraints| property to apply the \verb|IS JSON|
constraint for \verb|JSONField| (shown by line 12). The cx\_Oracle database
driver returns \verb|CLOB| and \verb|BLOB| values from the database as
\verb|LOB| objects in Python\cite{cxoracle:lob}. Meanwhile, the \verb|LOB|
objects need to be converted to Python strings in order to perform
deserialization. To do this, the \verb|get_db_converters()| method in the
\verb|DatabaseOperations| class is overridden so that \verb|JSONField| uses the
\verb|convert_textfield_value| converter, as shown by lines 22 and 23 of the
listing.\footnote{\code{TextField} also uses the \code{NCLOB} data type on
Oracle Database, so its converter can be reused for \code{JSONField}.}
