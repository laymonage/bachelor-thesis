...
The lookups are taken from the existing lookups for the PostgreSQL-only
\verb|JSONField| and modified to support the other database backends. The
lookups are the containment lookups (\verb|__contains| and
\verb|__contained_by|), as well as the key existence lookups (\verb|has_key|,
\verb|has_keys|, and \verb|has_any_keys|). In addition to that, the
\verb|__exact| lookup also has to be overridden in order to support the JSON
\verb|null| value.
...
unified as \verb|KeyTransform| in the implementation. The value extracted by
the transforms can be chained with some of the built-in lookups and the
\verb|JSONField|-specific lookups. As these transforms may return values of
different data types, some of the lookups also need to be adjusted.
