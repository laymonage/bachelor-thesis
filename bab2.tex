...
The object-relational mapping (ORM) system is one of Django's most complex
features. ORM is a programming technique that allows its users to query and
...
data models are represented as Python classes as shown in
\autoref{code:personmodel}. The model class also has attributes, called model
fields, which represent the columns of the database table as shown in
\autoref{table:myapp_person}. These model fields define the data types that are
used in the database. For example, the \code{CharField} model field uses the
\code{VARCHAR} data type in the database. In addition, these model fields also
define the behavior of the columns, such as their \code{NOT NULL} and
\code{UNIQUE} constraints in the database.
...
\noindent
\begin{minipage}{\linewidth}
\lstinputlisting[language=Python, caption={An excerpt of the previous
...
For example, it used the \code{Json} adapter included in psycopg2, the
...
in the \code{get\_prep\_value} method). In addition, it uses the \code{JSONB}
data type that's available on PostgreSQL, but not on other database backends
supported by Django.
