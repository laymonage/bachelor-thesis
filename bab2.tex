...
ORM is a programming technique that allows its users to query and
manipulate data stored in a relational database using an object-oriented
paradigm \cite{linskey:orm}. Object query, load, and delete operations are
translated into SQL \code{SELECT} and \code{DELETE} statements, while object
allocation and modification operations are translated into SQL \code{INSERT}
and \code{UPDATE} statements. To achieve this, the scalar values retrieved from
the database are converted into object instances in the program and vice versa.

\lstinputlisting[language=Python, caption={A \code{Person} model with a
\code{CharField} to store the name and a \code{DateField} to store the date
of birth.}, label=code:personmodel]{codes/2-personmodel.py}
...
data models are represented as Python classes. A model class also has
attributes, called model fields, which represent the columns of the database
table. These model fields define the data types and constraints that are used
in the database. For example, to store user information, a \code{Person} model
as shown in \autoref{code:personmodel} can be created. The \code{Person} model
consists of a \code{name} attribute as a \code{CharField} to store the name of
the user and a \code{dob} attribute as a \code{DateField} to store the user's
date of birth. The \code{Person} model represents the relational database table
shown in \autoref{table:myapp_person}. The table name is derived from the
Django application name and the model name. The table schema is derived from
the model fields: the \code{name} field corresponds to the \code{name} column
with the SQL \code{VARCHAR} data type, while the \code{dob} field corresponds
to the \code{dob} column with the SQL \code{DATE} data type and without the
...
\code{django.db.backends} module. In order to implement \code{JSONField} for
...
booleans, or \code{null}. However, they can also be JSON objects or JSON
...
Psycopg 2 supports the \code{jsonb} and \code{jsonb} data types as of version
...
\code{VARCHAR2} and \code{LOB} data types \cite{oracle:json}. Unlike the other
...
JSON objects or JSON arrays. JSON validation can be enforced using the \code{IS
JSON} condition as a \code{CHECK} constraint. Querying JSON data can be
...
implementation of \code{JSONField} has to provide the same programming
interface regardless which database backend is used. Therefore, it is important
to see how the programming interface for \code{JSONField} can be used by Django
programmers.
...
\code{IntegerField} and \code{TextField}, but there are also those that can
store fairly complex data such as \code{FileField} and \code{ImageField}.
...
\lstinputlisting[language=Python, caption={A Django model named \code{Profile}
	that stores a user's configurations in a \code{JSONField}.},
	label=code:profilemodel]{codes/2-profilemodel.py}

\noindent
\begin{minipage}{\linewidth}
\lstinputlisting[language=Python, caption={A Python shell session that
demonstrates how storing, modifying, and retrieving data in a
\code{JSONField} works.}, label=code:jsonshell]{codes/2-shell.py}
\end{minipage}

To use the \code{JSONField} model field, the programmer can define a Django
model and declare a field with an instance of \code{JSONField} as shown in
...
dictionary. When calling the \code{create()} method, the object inside the
...
\begin{minipage}{\linewidth}
\lstinputlisting[language=Python, caption={A Python shell session that
demonstrates how \code{JSONField} lookups and transforms work.},
label=code:shelllookups]{codes/2-shelllookups.py}
\end{minipage}

In addition to storing or accepting JSON-encoded data, \code{JSONField} also
includes \code{JSONField}-specific lookups and transforms that can be used to
...
\noindent
\begin{minipage}{\linewidth}
\lstinputlisting[language=Python, caption={An excerpt of the PostgreSQL-only
\code{JSONField} implementation.},
label=code:pgjsonfield]{codes/2-jsonfield.py}
\end{minipage}

Before this research, the existing implementation of \code{JSONField} relied on
...
\code{Json} adapter included in Psycopg 2. The adapter was used to encode a
Python object into a JSON-encoded string before it's sent to the database
(implemented in the \code{get\_prep\_value} method). In addition, it used the
\code{jsonb} data type that's available on PostgreSQL, but not on other
database backends supported by Django.

In order to reimplement \code{JSONField} for all of the database backends, the
...
\code{JSONField} can be used by Django developers. With this information, a new
implementation of \code{JSONField} for all of the database backends can be
designed.
